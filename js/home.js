...
async function searchFlights() {
    ...
    try {
        ...
        flights.forEach(flight => {
            const fromAirportName = flight.fromAirport?.name || 'مطار غير معروف';
            const toAirportName = flight.toAirport?.name || 'مطار غير معروف';
            const flightDiv = document.createElement('div');
            flightDiv.className = 'card mb-2';
            const flightDate = new Date(flight.flight_date);
            const options = { month: 'long', day: 'numeric' };
            const formattedDate = flightDate.toLocaleDateString('en-EG', options);
            
            flightDiv.innerHTML = `
                <div class="card-body hcel mt-1">
                    <h5 class="card-title">رحلة ${flight.flight_number}</h5>
                    <p class="card-text">
                        The date : ${formattedDate}<br>
                        The time : ${flight.launching_time} - ${flight.destination_time}<br>
                        Ticket price : $${flight.flight_price*numTravellers}<br>
                        Number of passengers : ${numTravellers}
                    </p>
                    <button class="btn btn-primary barbtnsafa" onclick="selectFlight(${flight.id}, ${flight.flight_price})">Reserve a seat</button>
                </div>
            `;
